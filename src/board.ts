...
export class Board implements Iterable<StateType> {
	...
	constructor(board?: BoardInput) {
		for (let index = 0; index <= this.length - 1; index += 1) {
			this[index as BoardPositions] = State.from(
				board?.[index] ?? (0 as StateKeys),
			);
		}
	}

	static getPosition(
		move: { p: BoardPositions } | { x: 0 | 1 | 2; y: 0 | 1 | 2 },
	): BoardPositions {
	...
	public get remaining(): number {
		...
		for (let i = 0; i < this.length; i++)
			if (this[i as BoardPositions].isEmpty()) n++;
		return n;
	}
	...
	public set(
		move: Parameters<typeof Board.getPosition>[number],
		state: MoveX | MoveO,
	): void {
		...
		if (!currentState.isEmpty())
			throw new Error(
				`The position ${'p' in move ? move.p : `x: ${move.x}, y: ${move.y}`} is already taken by ${currentState.toString()}`,
			);

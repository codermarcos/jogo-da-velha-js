...
describe('Game', () => {
	describe('start', () => {
		it('should initialize with an empty board', () => {
			...
			assert.strictEqual(
				...
				9,
				'Game should start with 9 remaining positions',
			);
			assert.deepStrictEqual(
				game.winner,
				undefined,
				'Game should not have a winner at start',
			);
		});
	...
	describe('play', () => {
		it('should allow players play properly', () => {
			const game = new Game();
			game.play({ p: 0 });
			assert.strictEqual(
				game.board[0].toString(),
				'X',
				'Player 1 should have played in position 0',
			);
			assert.strictEqual(
				game.next.toString(),
				'O',
				'Next player should be Player 2',
			);

			game.play({ p: 1 });
			assert.strictEqual(
				game.board[1].toString(),
				'O',
				'Player 2 should have played in position 1',
			);
			assert.strictEqual(
				game.next.toString(),
				'X',
				'Next player should be Player 1',
			);
			...
			try {
				...
				assert.strictEqual(
					error.message,
					'The position 1 is already taken by O',
					'Should no allow play when has winner',
				);
			}
		...
		it('should call onNext callback', () => {
			...
			assert.strictEqual(
				next?.remaining,
				8,
				'Board should have 8 remaining positions',
			);
		});
		...
		it('should call onFinish callback', () => {
			...
			assert.ok(
				called,
				'onFinish callback should be called when the game is over',
			);
			assert.equal(winner?.toString?.(), 'X', 'Game should has winner');
			assert.strictEqual(
				game.board.remaining,
				2,
				'Game should end remainig 2 moves',
			);
